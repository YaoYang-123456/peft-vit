...
- **层位置取决于偏移类型(核心发现)**:
  ...
  - 近零偏移(高分辨率 Flowers/Pets):层位置影响**消失**(<0.3 pp,无一致方向);
...
**层位置消融(LoRA,3 种子)**:CIFAR-100 上 early/mid/even ≈92.1–92.4% vs **late 90.91%**(SSF 同模式,late 90.40%);Flowers/Pets 各放法相近(<0.3 pp);**DTD 上反转:late 79.18% ≈ all-12 79.06% > early 77.09%**。
