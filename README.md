...
## 主要结论
- PEFT 仅用 **<0.5% 参数**即可逼近全量微调(在 CIFAR-100 上恢复其约 **99%** 精度);小数据上可匹敌经学习率调优的全量微调(默认 lr 下的“反超”主要来自未调参基线,见报告 §3.7)。
- **层位置取决于偏移类型(核心发现)**:
...
- LoRA **显存与速度最均衡**;SSF 训练显存反而高于全量微调——这主要是“在多个宽激活点上学习逐通道缩放须保留被缩放激活”的内在代价(可用梯度检查点缓解、难以靠融合或原地写法完全消除),而非纯粹的实现瑕疵(见报告 §3.6)。
